...
class EQTransformerMagV2(WaveformModel):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            labels=["magnitude_scalar"],  # Single scalar output for magnitude
        ...
        # Scalar magnitude prediction head (single value output)
        # Pool temporal features and predict scalar magnitude
        self.scalar_pool = nn.AdaptiveAvgPool1d(1)  # Pool to single value
    ...
    def forward(self, x, logits=False):
        ...
        x, _ = self.transformer_2(x)  # Shape: (batch, 16, time)

        # Scalar magnitude prediction
        # Pool temporal dimension: (batch, 16, time) -> (batch, 16, 1)
        pooled = self.scalar_pool(x)  # (batch, 16, 1)
        pooled = pooled.squeeze(-1)  # (batch, 16)
        
        # Predict scalar magnitude: (batch, 16) -> (batch, 1) -> (batch,)
        magnitude = self.scalar_head(pooled).squeeze(-1)  # (batch,)
    ...
    def annotate_batch_post(
        ...
        """Post-processing: ensure scalar output (no additional dimensions needed)."""
        # Model now outputs (batch,) shape directly, no modification needed
        return batch
