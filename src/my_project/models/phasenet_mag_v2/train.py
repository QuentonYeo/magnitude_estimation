...
def train_phasenet_mag(
    ...
    """
    Train PhaseNetMag model for scalar magnitude regression.
    
    The model predicts a single magnitude value per waveform.
    Training uses the maximum of the labeled magnitude values as the target,
    which corresponds to the true event magnitude (constant after P-arrival).
    ...
    """
    print("\n" + "=" * 50)
    print("TRAINING PHASENETMAG V2 (SCALAR PREDICTION)")
    print("=" * 50)
    print(f"Training PhaseNetMag on {data.__class__.__name__}")
    print(f"Output: 1 scalar magnitude per waveform")
    ...
    def train_loop(dataloader):
        ...
        for batch in pbar:
            ...
            y_pred = model(x_preproc)  # Shape: (batch,) - scalar output

            # Calculate loss
            loss = criterion(y_pred, y_scalar)
            ...
            with torch.no_grad():
                mae = torch.abs(y_pred - y_scalar).mean().item()
                total_mae += mae
    ...
    def validation_loop(dataloader):
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                # Calculate loss
                loss = criterion(y_pred, y_scalar)
                mae = torch.abs(y_pred - y_scalar).mean()
