...
class EQTransformer(WaveformModel):
    """
    The EQTransformer from Mousavi et al. (2020) with optional magnitude prediction
    ...
    :param predict_magnitude: If True, adds a magnitude prediction head
    :param magnitude_label: Label name for magnitude predictions (default: 'M')
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Update labels to include magnitude if requested
        labels = ["Detection"] + list(phases)
        if predict_magnitude:
            if magnitude_label not in labels:
                labels.append(magnitude_label)
        ...
        if self.predict_magnitude:
            # Use similar architecture to picking branches but with regression output
            if original_compatible == "conservative":
                self.magnitude_lstm = CustomLSTM(
                    ActivationLSTMCell, 16, 16, bidirectional=False
                )
            else:
                self.magnitude_lstm = nn.LSTM(16, 16, bidirectional=False)

            self.magnitude_attention = SeqSelfAttention(
                input_size=16, attention_width=3, eps=eps
            )

            self.magnitude_decoder = Decoder(
                input_channels=16,
                filters=self.filters[::-1],
                kernel_sizes=self.kernel_sizes[::-1],
                out_samples=in_samples,
            ...
            # Regression head (no sigmoid activation)
            self.magnitude_conv = nn.Conv1d(
                in_channels=self.filters[0], out_channels=1, kernel_size=11, padding=5
            )

    def forward(self, x, logits=False, return_magnitude=None):
        """
        Forward pass through the network.

        :param x: Input tensor
        :param logits: If True, return raw logits for detection and phase predictions
        :param return_magnitude: If True, return magnitude predictions. If None, uses self.predict_magnitude
        :return: Tuple of (detection, phase1, phase2, ..., [magnitude])
        """
        if return_magnitude is None:
            return_magnitude = self.predict_magnitude
        ...
        if return_magnitude and hasattr(self, "magnitude_lstm"):
            mx = x.permute(
                2, 0, 1
            )  # From batch, channels, sequence to sequence, batch, channels
            mx = self.magnitude_lstm(mx)[0]
            mx = self.dropout(mx)
            mx = mx.permute(
                1, 2, 0
            )  # From sequence, batch, channels to batch, channels, sequence
            mx, _ = self.magnitude_attention(mx)
            mx = self.magnitude_decoder(mx)
            # No sigmoid activation for regression
            magnitude_pred = self.magnitude_conv(mx)
            magnitude_pred = torch.squeeze(
                magnitude_pred, dim=1
            )  # Remove channel dimension

            outputs.append(magnitude_pred)
    ...
    def annotate_batch_post(
        ...
        # Handle both regular outputs and magnitude outputs
    ...
    def get_model_args(self):
        ...
        model_args["predict_magnitude"] = self.predict_magnitude
        model_args["magnitude_label"] = self.magnitude_label
